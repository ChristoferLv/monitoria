...
int is_in(int val, const char *v1)
{
    int n = strlen(v1);
    for (int i = 0; i < n; i++)
    {
...
char **split(const char *str, int *n, const char* separators)
{
    int numWords = 0;
    ...
    while (currentChar != '\0')
    {
        if (is_in(currentChar, separators))
        {
            sep = 1;
        }
...
char** get_words_sep(const char *path, const char* separators, int* words_count)
{
    ...
    char **palavras = split(conteudo, words_count, separators);
    printf("{");
...
int main()
{
    ...
    int numPalavras = -1;
    char* separators = " ,;.:?!-";
    resultado = get_words_sep("C:\\laragon\\teste3.txt", separators, &numPalavras);
